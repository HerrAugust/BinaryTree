	...
	// aggiunta di altri eventuali campi...
        ...
        public static BinaryTree<Integer> generaCasuale(int h) {
        	...
        	for(int i = 0; i < h && !queue.isEmpty(); i++) {
        		...
        		corrente.setValue(rand.nextInt(maxvalue));
        		if(rand.nextBoolean()) {
        			corrente.setLeft(new BinaryNode<Integer>(rand.nextInt(maxvalue)));
        			queue.enqueue(corrente.getLeft());
        		}
        		if(rand.nextBoolean()) {
        			corrente.setRight(new BinaryNode<Integer>(rand.nextInt(maxvalue)));
        			queue.enqueue(corrente.getLeft());
        		}
